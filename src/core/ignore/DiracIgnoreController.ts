...
export class DiracIgnoreController {
	...
	private async processIgnoreContent(content: string): Promise<void> {
		...
		const combinedContent = await this.processDiracIgnoreIncludes(content)
	 ...
	 */
	private async processDiracIgnoreIncludes(content: string): Promise<string> {
		let combinedContent = ""
